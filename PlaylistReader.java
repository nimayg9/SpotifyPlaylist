...
// -----------------------------------------------------------------------------
/**
 * This class parses the input data from two text files
 ...
 * 
 * (I only filled in the missing lines from the skeleton file)
 * 
    ...
    private Playlist[] readPlaylistFile(String playlistFileName)
        ...
        while (file.hasNextLine()
            ...
            if (tokens.length < 8) {
                ...
                throw new ParseException("Playlist file has incomplete data",
                    tokens.length);

            }
            else if (tokens.length > 8) {
                throw new ParseException("Playlist file has too much data",
                    tokens.length);
            }
                ...
                if (!isInValidPercentRange(minPop, minRock, minCountry)
                    ...
                    throw new DailyMixDataException(
                        "Genre values are out of bounds");
                }
        ...
        if (playlistCounter < PlaylistCalculator.NUM_PLAYLISTS) {
            ...
            throw new FileNotFoundException(
                "File has the incorrect amount of playlists");
        }
    ...
    private ArrayQueue<Song> readQueueFile(String songFileName)
        ...
        while (file.hasNextLine()) {
            ...
            if (tokens.length != SONG_TOKENS && tokens.length != SONG_TOKENS
                ...
                throw new ParseException("Song file has incomplete data",
                    tokens.length);
            }
            ...
            Song song = new Song(songName, pop, rock, country,
                suggestedPlaylist);
